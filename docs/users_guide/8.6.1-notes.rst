...
- SSE code can now be used in GHCi on Windows. (e.g. hmatrix etc.)
...
- The runtime linker on Windows has been overhauled to properly handle section
  alignment, lower the amount of wasted memory and lower the amount of in use
  memory and apply memory protection.
  See :ghc-ticket:`13617`. Note that committed memory may be slightly higher for
  now because the entire compiler has not been switched over to the new memory
  allocator yet.
